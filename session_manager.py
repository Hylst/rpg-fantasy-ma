...
from src.models.game_state import GameSession, GameStateEncoder
...
class SessionManager:
    ...
    def save_session(self, session: GameSession, save_type: str = "auto") -> bool:
        """
    ...
    def load_session(self, session_id: str, save_type: str = "auto") -> Optional[GameSession]:
        """
    ...
    def _reconstruct_session(self, session_data: dict) -> GameSession:
        """Reconstruit un objet GameSession depuis les données JSON"""
        # Note: Cette méthode nécessiterait une implémentation complète
        # de désérialisation pour reconstruire tous les objets complexes
        # Pour l'instant, on utilise une approche simplifiée
        
        # Ici on devrait reconstruire tous les objets dataclass
        # depuis les dictionnaires JSON. Pour simplifier, on utilise
        # une reconstruction basique.
        
        from src.models.game_state import (
            GameSession, Player, WorldState, Quest, 
            PlayerStats, InventoryItem, StatusEffect, NPC, Location, WorldEvent
        )
        
        # Cette implémentation est simplifiée et devrait être complétée
        # avec une désérialisation complète de tous les objets
        
        # Pour l'instant, retourner un objet basique
        # Dans une implémentation complète, il faudrait reconstruire
        # tous les objets dataclass récursivement
        
        session = GameSession(
            id=session_data["id"],
            player=Player(**session_data["player"]),  # Simplifié
            world=WorldState(**session_data["world"]),  # Simplifié
            quests=[Quest(**q) for q in session_data.get("quests", [])],  # Simplifié
            ...
            universe=session_data.get("universe", "fantasy"),
            narrative_style=session_data.get("narr
